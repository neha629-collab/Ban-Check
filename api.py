...
def check_player():
    ...
    region = request.args.get('region', 'IND').upper()  # Default to IND if not provided
    ...
    # Base cookies (you may need to refresh session_key and datadome periodically)
    cookies = {
    ...
    try:
        # Shop2game ID Check
        res = requests.post('https://shop2game.com/api/auth/player_id_login', cookies=cookies, headers=headers, json=json_data, timeout=10)
        ...
        # Ban check (Fixed region parameter using config['region_code'])
        ban_url = f"https://ff.garena.com/api/antihack/check_banned?lang=en&uid={target_id}&region={config['region_code']}"
